...
async function generateLLMProposals(projectPath, userIntent, metadata) {
  ...
  const systemPrompt = `Eres un asistente de código experto. Based on the user's request and project metadata, generate code changes.
...
Respond with a JSON array of file changes. Each change should have:
- file: relative file path
- type: "create" | "update" | "delete"
- content: the full file content for create/update

Respond ONLY with valid JSON, no other text.`;
  
  const userPrompt = `User request: ${userIntent}

Generate the necessary code changes to fulfill this request. Consider the project structure and best practices for ${metadata.language}/${metadata.framework || 'vanilla'}.`;
  ...
  try {
    ...
    // Parse JSON response
    const changes = JSON.parse(response.content);
